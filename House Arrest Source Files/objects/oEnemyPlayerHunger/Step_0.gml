...
if oPlayer.x+pl_distance < x { kLeft = true; } else { kLeft = false; }
if oPlayer.x-pl_distance > x { kRight = true; } else { kRight = false; }
...
if alarm[2]=-1 and !kLeft and !kRight { kAction=true; alarm[2]=60; } else { kAction=false }
kBlock       = false
kRollL       = false
kRollR       = false
...
if (!kBlock && kAction) {
    if (!attacking) {
        ...
        // Jab in place
            ...
            image_index  = 0;
            image_speed  = 0.33;
            sprite_index = sHungerJab;
            
			with (instance_create(x+(facing*170), y, oEnemyAttack)) { facing=other.facing; hunger_damage=10;
			}
...
// Jab
if (sprite_index == sHungerJab && round(image_index) > 0) {    
    with (instance_create(x, y, oPlayerAtkBox)) {
